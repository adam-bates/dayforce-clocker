...
exports.handler = function(context, event, callback) {
  ...
  if (requestType === REQUEST_TYPE.BOTH) {
    ...
    return callback(null, response(
      "Please specify either 'in' or 'out'."));
  }
  
  const url = "https://dayforce-clocker.herokuapp.com";
  ...
  if (requestType === REQUEST_TYPE.PROBE) {
    ...
    instance
      .post(action)
      .then(res => {
        return callback(null, response("Wait a minute for the screenshot to upload: " + res.data.screenshotS3Url));
      })
      .catch(err => callback(null, response("Got error: " + err)));
  }
};